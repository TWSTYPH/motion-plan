...
void rcvPointCloudCallBack(const sensor_msgs::PointCloud2 & pointcloud_map)//传回ros中的点云数据
{   
    if(_has_map ) return;
    ...
    pcl::fromROSMsg(pointcloud_map, cloud);//ROS中的点云数据转换成PCL点云数据类型
    
    if( (int)cloud.points.size() == 0 ) return;//如果点云数据为0推出函数
